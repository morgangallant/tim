...
import {
  JSONToObj,
  WritePrefixedCounterValue,
  GetLastPrefixedCounterSet,
  GetLastPrefixedCounterValue,
} from "./helpers";
...
async function LookupTelegramUser(wh: TelegramWebhook): Promise<User> {
  ...
  if (userid == null) {
    ...
    user = await JSONToObj(await TIMDB.get(`users:${userid}`, "json"), User);
  }
  if (!user.firstname) {
 ...
 */
async function ExtractIntentAndEntities(
  query: string
): Promise<{
  intent: NLIntent;
  activity?: Activity;
...
function GetReferenceTimestamp(): number {
  ...
  const yesterday_1030 = midnight.getTime() - 1.5 * MSInHour;
  // If it is between 10:30pm-12:00pm, we don't shift to midnight and subtract.
  if (
    moment().tz("America/New_York").toDate().getTime() - yesterday_1030 >
    24 * MSInHour
  ) {
    const now = moment().tz("America/New_York").hour(22).minute(30);
    return now.toDate().getTime();
  }
  return yesterday_1030;
}
 ...
 */
async function GetActivitySummaryForDay(
  user: string
  ...
  const log = (
    await GetLastPrefixedCounterSet(
      prefix,
      NLLog,
      (v: NLLog): boolean => {
...
async function NLHandleDailySummary(r: NLRequest): Promise<NLResponse> {
  ...
  for (let [activity, rd] of summary) {
    ...
    msg += `${activity}: ${FormatNumberAsString(
      rd
    )}h  (goal: ${FormatNumberAsString(gd)}h)\n`;
  }
...
async function GetUserCurrentActivity(user: string): Promise<Activity> {
  const prefix = `users:${user}:interactions`;
  const last = await GetLastPrefixedCounterValue(
    prefix,
    NLLog,
    (v: NLLog): boolean => {
